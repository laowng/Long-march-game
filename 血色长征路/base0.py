...
class Game:
    ...
    def has_won(self) -> bool:
        ...
        """
        S=self._grid.serialize().values()
        return HOSPITAL not in S and TRACKING_ZOMBIE not in S and ZOMBIE not in S
...
class Zombie(Entity):
    ...
    def step(self, position: Position, game: Game) -> None:
        ...
        if destinations:
            game.get_grid().move_entity(position, destinations[0])
...
class Pickup(Entity):
    ...
    def __init__(self):
        ...
        self._using = True     # Entities are not selected to be actively
...
class Inventory:
    ...
    def add_item(self, item: Pickup) -> None:
        ...
        """
        if self.contains(item.display()):
            for p in self._items:
                if p.display() == item.display():
                    p._lifetime+=item.get_lifetime()
                    break
        else:
        ...
        if self._player_position is not None:
            ...
            if self._grid.in_bounds(destination):
                ...
                if entity is not None and isinstance(entity, Pickup):
                    ...
                    return
                elif entity is not None and isinstance(entity, Hospital):
                    S = self._grid.serialize().values()
                    if TRACKING_ZOMBIE in S or ZOMBIE in S:
                        return
...
class TimeMachine(Pickup):
    ...
    def get_durability(self) -> int:
        """
        Return the durability of a TIME_MACHINE.

        A player can only hold a garlic entity for 1e5(infinite) _steps_.
        """
        return int(1)

    def display(self) -> str:
        """
        Return the character used to represent the TIME_MACHINE entity in
        a text-based grid.

        A TIME_MACHINE should be represented by the 'M' character.
        """
        return TIME_MACHINE
    def hold(self) -> None:
        """
...
class GreatMapLoader(AdvancedMapLoader):
    """
    add Time Machine
    """
    ...
    def create_entity(self, token: str) -> Entity:
        if token == TIME_MACHINE:
            return TimeMachine()
        return super().create_entity(token)
def great_game(filename: str) -> AdvancedGame:
    """
    Return an initialised advanced game corresponding to task 3
    in assignment two.
    """
